...
Mule.prototype.error = function(s) {
	if (this.errorElem) this.errorElem.remove();
	var self = this;
...
Mule.prototype.query = function(ignore_cache) {
	...
	function onFail() {
		...
		if (self.fails < 5) {
			...
			self.error('request failed too many times');
		}
	}
	...
	function onResponse(data) {
		...
		function watchProgress(percent) {
			if (typeof percent != 'string') {
				self.error('migration failed, retrying...')
				setTimeout(function() { self.reload() }, 10000)
				return
			}
			if (percent == '100') {
				self.reload()
		...
		if (res.Migrate) {
			self.error('migration required')
			return

			window.realmAPI('migrate/doMigration', CR, function(xhr) {
				xhr.fail(onFail).done(function(data) {
					watchProgress('0')
				})
			})
		...
		if ('TOSPopup' in res) {
			window.realmAPI('account/acceptTOS', CR)
		}

		if (res.Account && res.Account.IsAgeVerified != 1) {
			CR.isAgeVerified = 1
			window.realmAPI('account/verifyage', CR)
		}
