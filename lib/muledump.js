...
function addstar($t, d) {
	...
	if (r < 1) return;
	var $s = $('<span>').text('\u2605').addClass('star');
	$s.css('color', STARCOLOR[Math.floor(r / 13)] || 'lime');
	$s.appendTo($t);
	$('<span>').text(r).appendTo($t);
}
...
function checkversion() {
	function checkupd(data) {
		...
		if (!url) return;
		var link = $('<a>').text('download v' + topver);
		link.attr('href', url).appendTo('#top');
	}
	$.ajax({
		dataType: 'jsonp',
